...
3. Trigger paths: right-click context menu on a selection/editable field, or `Ctrl+Shift+L` (mac: `Cmd+Shift+L`) — bound in `manifest.json` under `commands.revise-in-place`. `Ctrl+Shift+U` (`commands.undo-revise`) or the "Son revizyonu geri al" context menu item undoes the last in-place revision.
...
- Badge text on `chrome.action` is the primary user feedback for the in-place flow (`…`, `✓`, `kopya`, `key`, `hata`, `?`, `↩`). Always pair `setBadge` with `clearBadgeLater` to avoid sticky badges.
...
- `README.md` — user-facing install/usage (Turkish).
